...
- Application preparation note: derives three preparation prompts from the real form fields, states that the page does not auto-save, and explains that all answers can be reviewed and edited before the network request. It does not claim an average completion time.
- Application status ledger: `작성 → 제출 전 확인 → 접수 완료 → 내부 심의 → 8/15까지 안내 대상자 연락`. It also preserves the current policy that non-selected applicants receive no separate notice; it must not imply universal result notification, a receipt ID, or a receipt timestamp.
